-- Queries for Categories and Competitions
...
SELECT cat.category_name, COUNT(c.competition_id) AS number_of_competitions
FROM Categories cat
...
SELECT p.competition_name AS parent_competition, s.competition_name AS sub_competition
FROM Competitions p
LEFT JOIN Competitions s ON p.competition_id = s.parent_id
WHERE p.parent_id IS NULL;

-- Analyze the distribution of competition types by category:

SELECT cat.category_name, c.type AS competition_type, COUNT(*) AS count
FROM Competitions c
JOIN Categories cat ON c.category_id = cat.category_id
